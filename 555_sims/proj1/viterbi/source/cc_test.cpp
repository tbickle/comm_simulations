...
int main(void)
{
	...
	vi b={1,1,0};
	vi b_coded={1,0,1,0,0,1,1,0,1};
	reverse(b_coded.begin(),b_coded.end());
...
*/
	...
	for(int g=0;g<b_coded.size();g++) { // for discrete time t (up to T)
		...
		if(w_iter==(nv-1)) {
			//for(int a=0;a<word.size();a++) cout<<word[a]; cout<<endl; // test
			for (int h=0;h<cc.S();h++) { // analyze inputs going into each state
				int tran, d, in, pstate;
				...
				d1=d1+sp[pS[h][1]][0];
				// choose min.
				if(d0<=d1) {tran=0;d=d0;in=pI[h][0];pstate=pS[h][0];}
				else {tran=1;d=d1;in=pI[h][1];pstate=pS[h][1];}
				// log data
				temp[h]=d;
				temp1[h]=pstate;
/*
				cout<<"into state:"<<h<<":"<<endl;
				cout<<"PS:"<<pS[h][0]<<"->"<<tran0<<"/"<<pI[h][0]<<"="<<d0<<endl;
				cout<<"PS:"<<pS[h][1]<<"->"<<tran1<<"/"<<pI[h][1]<<"="<<d1<<endl;
				cout<<"min_dist:"<<d<<endl;
				cout<<endl;
*/
			} // end_for
			for(int h=0;h<cc.S();h++) {sp[h][0]=temp[h]; sp[h][b_iter]=temp1[h];} // list acc. dist & surv. path PS's transitioning into h state
			b_iter++;
			w_iter=0;
		} // end_if
		else {w_iter++;}
