...
namespace gr {
	...
	fsm::fsm() {
		d_I=0; d_S=0; d_O=0; d_NS.resize(0); d_OS.resize(0);
		d_PS.resize(0); d_PI.resize(0);
		d_TMi.resize(0); d_TMl.resize(0);
	}

	fsm::fsm(const fsm &FSM)
	{
		d_I=FSM.I(); d_S=FSM.S(); d_O=FSM.O(); d_NS=FSM.NS(); d_OS=FSM.OS();
		d_PS=FSM.PS(); /* is this going to make a deep copy? */ d_PI=FSM.PI();
		d_TMi=FSM.TMi(); d_TMl=FSM.TMl();
	}

	fsm::fsm(int I, int S, int O, const std::vector<int> &NS, const std::vector<int> &OS)
	{
		std::cout<<"Supplying Parameters (I,S,O,NS,OS)\n";
		d_I=I; d_S=S; d_O=O; d_NS=NS; d_OS=OS; // private variables
	...
	fsm::fsm(const char *name)
	{
		...
		if((fsmfile=fopen(name,"r"))==NULL) throw std::runtime_error ("fsm::fsm(const char *name): file open error\n"); //printf("file open error in fsm()\n");
		if(fscanf(fsmfile,"%d %d %d\n",&d_I,&d_S,&d_O) == EOF) {
			if(ferror(fsmfile) != 0) throw std::runtime_error ("fsm::fsm(const char *name): file read error\n");
		}
	...
	fsm::fsm(int k, int n, const std::vector<int> &G)
	{
		...
		for(int i=0;i<k;i++) {
			for(int j=0;j<n;j++) {
				int mem = -1;
				if(G[i*n+j]!=0) mem=(int)(log(double(G[i*n+j]))/log(2.0));
				if(mem>max_mem_x[i]) max_mem_x[i]=mem;
				if(mem>max_mem) max_mem=mem;
			}
		}
		//printf("max_mem_x\n");
		if(pr) printf("\n");
		//for(int j=0;j<max_mem_x.size();j++) printf("max_mem_x = %d\n",max_mem_x[j]); //printf("\n");
		...
		for(int j=0;j<k*n;j++) {
			...
			dec2base(G[j],2,Gb[j]);
			if(pr) printf("g%d = ",j+1);
			if(pr) {for(int m=0;m<Gb[j].size();m++) printf("%d ",Gb[j][m]); printf("\n");}
		}
		...
		//printf("bases_x\n");
		//for(int j=0;j<max_mem_x.size();j++) printf("bases_x = %d\n",max_mem_x[j]); //printf("\n");
		d_NS.resize(d_I*d_S);
		d_OS.resize(d_I*d_S);
		std::vector<int> sx(k);
		...
		for(int j=0;j<k;j++) tb[j].resize(max_mem+1);
		...
		for(int s=0;s<d_S;s++) {
			...
			for(int i=0;i<d_I;i++) {
				...
				// evaluate next state
				for(int j=0;j<k;j++) nsx[j] = (inb[j]*bases_x[j]+sx[j])/2; 	// next state (for each shift register) MSB first
				d_NS[s*d_I+i]=bases2dec(nsx,bases_x);				// collect all values into the new state

				// evaluate transitions
				for(int j=0;j<k;j++) tx[j] = inb[j]*bases_x[j]+sx[j];		// transition (for each shift register) MSB first
				if(pr) printf("tran = ");
				for(int j=0;j<k;j++) {
					dec2base(tx[j],2,tb[j]);
					if(pr) {for(int m=0;m<tb[j].size();m++) printf("%d",tb[j][m]);
					printf(", ");}
				}
				...
				for(int nn=0;nn<n;nn++) {
					...
					for(int j=0;j<k;j++) {
						...
						for(int m=0;m<max_mem+1;m++) outb[nn] = (outb[nn] + Gb[j*n+nn][m]*tb[j][m]) % 2;
						if(pr) printf("%d",outb[nn]);
					}
				}
				if(pr) printf("\n");
		...
		if(pr) cout << endl << endl;

/*
		// MY TEST
		cout << "My tests:\n";
		int my_state, my_in, my_os, my_ns;
		//int init = 0; // initial state

		cout << "input data = 11000\n";
		vector<int> data(5);
		data = {1,1,0,0,0};

		my_state=0;
		for(int a=0;a<data.size();a++) {
			my_in=data[a]; my_os=d_OS[my_state*d_I+my_in]; my_ns=d_NS[my_state*d_I+my_in];
			printf("state:%d, in:%d -> out:%d, next_state:%d\n",my_state,my_in,my_os,my_ns);
			my_state=my_ns;
		}
		// end_my_test
*/
...
// **********************
	...
	void fsm::generate_PS_PI()
	{
		...
		for(int i=0;i<d_S;i++) {
			...
			int j=0;
			for(int ii=0;ii<d_S;ii++) { // # of states
				for(int jj=0;jj<d_I;jj++) { // # of alphabet chars
					if(d_NS[ii*d_I+jj]!=i) continue;
					d_PS[i][j]=ii;
					d_PI[i][j]=jj;
					//std::cout<<"d_PS["<<i<<"]["<<j<<"]="<<ii; // test
					//std::cout<<"   d_PI["<<i<<"]["<<j<<"]="<<jj; // test
					//std::cout<<"\n"; // test
					j++;
				}
			}
