...
int main(int argc, char *argv[])
{
	...
	int k=log2(M), K=k*pow(10,ORD), ns=K/k, err=0, ns2=0, dchk=0;
	double N0=0;
	// vectors
	vi EbN0dB(MAX), bs(K), sym(ns), sym_est(ns), sym_err(ns);
	...
	for(int z=0; z<MAX; z++) {
		...
		// channel
		N0 = 1/(1.0*k)*pow(10,EbN0dB[z]*-1/(1.0*10))*1;
		...
		err=0; ns2=0; fill(sym_err.begin(),sym_err.end(),0);
		for (int y=0; y<ns; y++) {
			vd temp(M);
			for (int x=0; x<M; x++) {temp[x] = abs(sym_tbl[x]-r[y]);}
			sym_est[y] = distance(temp.begin(),min_element(temp.begin(),temp.end()));
	...
	if(K<=k*100) display(EbN0dB,sym_tbl,bs,sym,s,n,r,sym_est,sym_err);
